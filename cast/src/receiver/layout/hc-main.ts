...
export class HcMain extends HassElement {
  ...
  private async _handleGetStatusMessage(msg: GetStatusMessage) {
    if (
      ...
      this._error = "Not connected to the same KS Assistant instance.";
      this._sendError(
  ...
  private async _handleShowLovelaceMessage(msg: ShowLovelaceViewMessage) {
    ...
    if (
      ...
      this._sendStatus(msg.senderId!);
      this._error =
        "Cannot show Lovelace because we're not connected to the same KS Assistant instance.";
      this._sendError(
        ReceiverErrorCode.WRONG_INSTANCE,
        this._error,
        msg.senderId!
  ...
  private _getErrorMessage(error: number): string {
    switch (error) {
      case 1:
        return "Unable to connect to the KS Assistant websocket API.";
      case 2:
        return "The supplied authentication is invalid.";
      case 3:
        return "The connection to KS Assistant was lost.";
      case 4:
        return "Missing hassUrl. This is required.";
      case 5:
        return "KS Assistant needs to be served over https:// to use with KS Assistant Cast.";
      default:
